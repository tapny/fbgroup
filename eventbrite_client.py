...
member_events = get_member_events

def gen_eb_member_key(attendee):
    return "%s-%s" % (attendee["first_name"], attendee["last_name"])


def get_member_events():
    # Pull all eb events.
    all_events = eb_client.user_list_events({"only_display":"id"})["events"]
    member_events = {}
    # Construct dictionary of key: member -> val []: events
    for event in all_events:
        eid = event["id"]
        attendees = eb_client.event_list_attendees({"id":eid})['attendees']
        for attendee in attendees:
            member_key = gen_eb_member_key(attendee)
            events = member_events.get(member_key, [])
            events.append(eid)
            member_events[member_key] = events
